...
func getStudents(c *gin.Context) {
	yearQuety := c.Query("year")
	if yearQuety != "" {
		filter := []Student{}
		for _, student := range students {
			if fmt.Sprintf("%d", student.Year) == yearQuety {
...
func createStudents(c *gin.Context) {
	...
	if newStudent.Year < 1 || newStudent.Year > 4 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "yeat must be 1-4"})
		return
	}
...
func updateStudents(c *gin.Context) {
	...
	for i, student := range students {
		if student.ID == id {
			...
			c.JSON(http.StatusOK, updateStudents)
			return
		}
	}
	c.JSON(http.StatusNotFound, gin.H{"error": "student not found"})
...
func deleteStudents(c *gin.Context) {
	...
	for i, student := range students {
		if student.ID == id {
			...
			c.JSON(http.StatusOK, gin.H{"message": "student deelete successfully"})
			return
		}
	}
	c.JSON(http.StatusNotFound, gin.H{"error": "student not found"})
...
func main() {
	...
	api := r.Group("/api/v1")
	{
		...
		api.POST("/students/:id", createStudents)
