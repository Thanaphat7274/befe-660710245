...
// @Description Return list of distinct categories from books
// @Tags Categories
// @Produce json
...
// @Summary Search books by category or term
// @Description Search books by category (?category=...) or q (?q=...) or both
// @Tags Books
// @Produce json
...
func getBooksByCategory(c *gin.Context) {
	...
	base := `SELECT id, title, author, isbn, year, price, created_at, updated_at FROM books`
	var where []string
	var args []interface{}

	if category != "" {
		where = append(where, "LOWER(category) = LOWER($"+strconv.Itoa(len(args)+1)+")")
		args = append(args, category)
	}
	if q != "" {
		where = append(where, "(LOWER(title) LIKE LOWER($"+strconv.Itoa(len(args)+1)+") OR LOWER(author) LIKE LOWER($"+strconv.Itoa(len(args)+2)+"))")
...
// @Description Return a small selection of featured books (random 3)
// @Tags Books
// @Produce json
// @Success 200 {array} Book
// @Failure 500 {object} ErrorResponse
...
func getFeaturedBooks(c *gin.Context) {
	rows, err := db.Query(`SELECT id, title, author, isbn, year, price, created_at, updated_at FROM books ORDER BY RANDOM() LIMIT 3`)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// @Description Return books that have a discount > 0 (requires a discount column)
// @Tags Books
// @Produce json
// @Success 200 {array} Book
// @Failure 500 {object} ErrorResponse
// @Router /books/discounted [get]
func getBooksOnDiscounted(c *gin.Context) {
	// If your DB has a discount column, use it; otherwise, return empty list
	rows, err := db.Query(`SELECT id, title, author, isbn, year, price, created_at, updated_at FROM books WHERE COALESCE(discount,0) > 0 ORDER BY discount DESC`)
	if err != nil {
		// If column doesn't exist, return empty array instead of crashing
		if strings.Contains(err.Error(), "column \"discount\"") {
			c.JSON(http.StatusOK, []Book{})
			return
		}
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
...
func main() {
	...
	api := r.Group("/api/v1")
	{
		...
		api.GET("/books/search", getBooksByCategory)
		api.GET("/books/featured", getFeaturedBooks)
		api.GET("/books/discounted", getBooksOnDiscounted)
	}
